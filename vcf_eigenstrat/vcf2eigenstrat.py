...
def parse_options():
    ...
    """
    options = {"vcf_eigenstrat": None, "out": "out", "ref": None, "indAsPop": False, "indmap": None}

    try:
        opts, args = getopt.getopt(sys.argv[1:], "v:o:r:i:", ["vcf_eigenstrat", "out", "ref", "indmap", "indAsPop"])
        print(opts, args)
    ...
    for o, a in opts:
        print(o, a)
        if o in ["-v", "--vcf_eigenstrat"]:
            options["vcf_eigenstrat"] = a
        if o in ["-r", "--ref"]:
            options["ref"] = a
        if o in ["-i", "--ind"]:
            options["indmap"] = a
        if o in ["--indAsPop"]:
            options["indAsPop"] = True
        elif o in ["-o", "--out"]:
            options["out"] = a
...
def main(options):
    """
    Convert vcf_eigenstrat to eigenstrat format (ind, snp and geno files)
    """
    vcf = open2(options["vcf_eigenstrat"])
